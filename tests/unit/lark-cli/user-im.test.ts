...
describe('directory user search', () => {
  it('uses user identity, distinguishes external people and parses the real envelope', async () => {
    const exec = stub(args => {
      expect(args).toContain('--as'); expect(args).toContain('user'); expect(args).toContain('张三');
      return { stdout: JSON.stringify({ ok: true, data: { users: [{ open_id: 'ou_1', localized_name: '张三', is_cross_tenant: true }], has_more: true } }) };
    });
    const r = await searchDirectoryUsers(ctx, { query: '张三' }, exec);
    expect(r.users).toEqual([{ id: 'ou_1', name: '张三', external: true, department: '' }]);
    expect(r.hasMore).toBe(true);
  });
  it('shows permission errors instead of treating them as no matches', async () => {
    await expect(searchDirectoryUsers(ctx, { query: 'a' }, stub(() => ({ code: 1, stderr: JSON.stringify({ ok: false, error: { message: 'scope missing' } }) })))).rejects.toThrow('scope missing');
  });
  it('rejects malformed responses and invalid ids', async () => {
    await expect(searchDirectoryUsers(ctx, {}, stub(() => ({ stdout: 'oops' })))).rejects.toThrow('格式');
    await expect(searchDirectoryUsers(ctx, { ids: ['--all'] }, stub(() => ({})))).rejects.toThrow('选择无效');
  });
});
