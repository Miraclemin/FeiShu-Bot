...
describe('secrets getter wrapper platform output', () => {
  ...
  it.each(['darwin', 'win32'] as const)('runs the bundled CLI in Node mode on Electron %s', async platform => {
    const root = await tmpRoot();
    const path = await ensureSecretsGetterWrapper({ rootDir: root, secretsGetterScript: join(root, 'secrets-getter') }, { platform, nodePath: 'electron', bridgeEntry: '/app/dist/cli.js', electron: true });
    const content = await readFile(path, 'utf8');
    expect(content).toContain('ELECTRON_RUN_AS_NODE=1');
    expect(content).toContain('/app/dist/cli.js');
    expect(content).not.toContain('desktop-host');
  });
