...
describe('profile-aware account and config commands', () => {
  it.each(['read-only', 'workspace', 'full'])('saves runtime permission %s only to the active profile', async (value) => {
    vi.useFakeTimers();
    const h = await createHarness();
    const before = await readRoot(h.rootDir);
    await h.command('/config submit', { runtime_access: value, message_reply: 'text' });
    const root = await waitForRoot(h.rootDir, r => r.profiles.claude?.permissions.defaultAccess === value && r.profiles.claude?.preferences.messageReply === 'text');
    expect(root.profiles.claude?.permissions).toEqual({defaultAccess:value, maxAccess:value});
    // Upstream serializes derived sandbox/permissionSource fields away; compare effective profiles.
    expect(normalizeProfileConfig(root.profiles['codex-dev'])).toEqual(normalizeProfileConfig(before.profiles['codex-dev']));
    expect(root.profiles.claude?.accounts).toEqual(before.profiles.claude?.accounts);
    await h.command('/config submit', { message_reply: 'markdown' });
    const retained = await waitForRoot(h.rootDir, r => r.profiles.claude?.preferences.messageReply === 'markdown');
    expect(retained.profiles.claude?.permissions.defaultAccess).toBe(value);
  });
