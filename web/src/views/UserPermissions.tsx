...
export function UserPermissions({ profile, refreshKey }: { profile: string; refreshKey: number }) {
  ...
  const ready = granted('contact:user:search') && granted('im:chat:read') && granted('im:chat.members:write_only');
  ...
  async function authorize() {
    ...
    try { setLogin(await apiPost<DeviceLogin>('/api/auth/login/start', { profile, scopes: ['contact:user:search', 'im:chat:read', 'im:chat.members:write_only'] })); }
  ...
  async function complete() {
    ...
    try {
      ...
      await refresh(); setLogin(null); window.dispatchEvent(new CustomEvent('workbench-auth-changed', { detail: profile }));
    } catch (e) { setError((e as Error).message); } finally { setBusy(false); }
  }
  return <div id="user-permissions" className="space-y-3 border-t pt-4">
    ...
    {login && <div className="space-y-3 rounded-lg border bg-muted/20 p-4 text-center">
      <p className="text-sm">一次授权人员搜索、查看群与添加群成员</p>
