...
export interface ConfigFormOpts {
  ...
  agentKind: AgentKind;
  runtimeAccess?: 'read-only' | 'workspace' | 'full';
      ...
      elements: [
        ...
        { tag: 'hr' },
        {
          ...
          elements: [
            ...
            { tag: 'hr' },
            { tag: 'markdown', content: '**电脑执行权限**\n影响当前机器人在所有群和私聊中的任务，与个人/团队模式和允许群名单独立。保存后下次任务生效；项目工作目录不等于读取范围。' },
            { tag: 'select_static', name: 'runtime_access', initial_option: opts.runtimeAccess ?? 'full', options: [
              {text:{tag:'plain_text',content:'只读：限制文件写入'},value:'read-only'},
              {text:{tag:'plain_text',content:'限制写入范围：默认仅工作目录及额外允许目录'},value:'workspace'},
              {text:{tag:'plain_text',content:'不限制项目目录（Full）：按本机账号权限访问'},value:'full'},
            ] },
            { tag: 'hr' },
            {
              tag: 'markdown',
      ...
      elements: [
        {
          ...
          content:
            ...
            `**电脑执行权限**: ${opts.runtimeAccess ?? "保持原设置"}（该机器人所有群；下次任务生效）
