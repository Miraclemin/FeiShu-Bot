...
interface ResumeCandidate {
  scopeId: string;
  agentId: import('../agent/catalog').AgentKind;
  cwdRealpath: string;
...
async function handleStatus(args: string, ctx: CommandContext): Promise<void> {
  ...
  if (['codex', 'inspector', 'product-manager'].includes(ctx.controls.profile)) {
    ...
    catch { projectBinding = '项目绑定暂时读取失败，请用 /project show 重试。'; }
  }
...
async function showConfigForm(ctx: CommandContext): Promise<void> {
  ...
  const card = configFormCard({
    runtimeAccess: ctx.controls.profileConfig.permissions.defaultAccess,
...
async function submitConfig(ctx: CommandContext): Promise<void> {
  const fv = ctx.formValue ?? {};
  const rawAccess = String(fv.runtime_access ?? '').trim();
  if (rawAccess && !['read-only', 'workspace', 'full'].includes(rawAccess)) {
    await reply(ctx, '电脑执行权限无效，未保存。请重新打开 /config。'); return;
  }
  const runtimeAccess = (rawAccess || ctx.controls.profileConfig.permissions.defaultAccess) as ProfileConfig['permissions']['defaultAccess'];
  const permissionChange = rawAccess && (runtimeAccess !== ctx.controls.profileConfig.permissions.defaultAccess || runtimeAccess !== ctx.controls.profileConfig.permissions.maxAccess);
  if (permissionChange && ctx.activeRuns.scopes().length) {
    await reply(ctx, '当前机器人还有任务运行，执行权限尚未修改。请等任务结束后重新提交。'); return;
  }
  ...
  void (async () => {
    ...
    try {
      ...
      await savePreferencesConfig(ctx, nextPreferences, requireMentionInGroup, larkCliIdentity, mode, rawAccess ? runtimeAccess : undefined);
    } catch (err) {
...
async function savePreferencesConfig(
  ...
  larkCliIdentity: ProfileConfig['larkCli']['identityPreset'],
  mode: ProfileMode,
  runtimeAccess?: ProfileConfig['permissions']['defaultAccess'],
): Promise<void> {
