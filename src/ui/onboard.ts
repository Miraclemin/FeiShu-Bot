import { isAgentKind } from '../agent/catalog';
...
export async function onboardCreate(body: unknown, rootDir?: string) {
  const fv = asRecord(body);
  if (!isAgentKind(fv.agentKind)) throw new HttpError(400, '请选择已安装的 Agent');
  const agentKind: AgentKind = fv.agentKind;
  if (!(await detectInstalledAgents()).some(a => a.kind === agentKind)) throw new HttpError(400, '本机未安装所选 Agent');
...
export async function writeNewProfile(
  ...
  profileConfig.workbench = { revision: 0, protectDocuments: true, groups: {} };
  profileConfig.larkCli.identityPreset = 'bot-only';
