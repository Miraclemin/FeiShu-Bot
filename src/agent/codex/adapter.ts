...
import { discoverSkills, selectedSkills, skillPrompt, codexSkillRuntime } from '../workbench-skills';
...
export class CodexAdapter implements AgentAdapter {
  ...
  run(opts: AgentRunOptions): AgentRun {
    ...
    const sourceHome = this.codexHome || (this.inheritCodexHome ? process.env.CODEX_HOME || join(homedir(), '.codex') : join(this.profileStateDir, 'codex-home'));
    const inventory = opts.skills ? discoverSkills(opts.cwd, [join(sourceHome, 'skills')]) : [];
    const selected = opts.skills ? selectedSkills(opts.skills, inventory) : [];
    const skillRuntime = opts.skills ? codexSkillRuntime(inventory, sourceHome, this.ignoreUserConfig) : undefined;
    const args = buildCodexArgs({
      cwd: opts.cwd,
      sandbox: opts.sandbox ?? this.sandbox,
      reviewCommands: this.larkChannel?.profile === 'product-manager' && (opts.sandbox ?? this.sandbox) === 'workspace-write',
      threadId: opts.threadId,
      images: opts.images,
      ignoreUserConfig: opts.skills ? false : this.ignoreUserConfig,
    ...
    if (skillRuntime) Object.assign(envOverrides, skillRuntime.env);
    ...
    child.stdin.end(prefixBridgeSystemPrompt((opts.skills ? skillPrompt(selected) : '') + opts.prompt, this.botIdentity), 'utf8');
