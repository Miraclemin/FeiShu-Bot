...
export type AgentCapabilityId = import('./catalog').AgentKind;
...
export function capabilityFor(profile: ProfileConfig): AgentCapability {
  if (profile.agentKind === 'claude') return claudeCapability(profile);
  const capability = codexCapability(profile);
  return { ...capability, agentId: profile.agentKind };
}
