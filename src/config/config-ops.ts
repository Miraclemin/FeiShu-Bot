...
export async function savePreferencesConfig(
  ...
  if (runtimeAccess !== undefined && !['read-only', 'workspace', 'full'].includes(runtimeAccess)) throw new Error('Invalid runtime access');
  const larkCli = {
  ...
  await withConfigFileLock(state.configPath, async () => {
    const root = await loadRootConfig(state.configPath);
    if (!root) {
      if (runtimeAccess !== undefined) throw new Error('权限设置需要profile格式配置；请先迁移旧配置');
    ...
    root.profiles[state.profile] = {
      ...profile,
      mode,
      ...(runtimeAccess !== undefined ? {permissions:{...profile.permissions,defaultAccess:runtimeAccess,maxAccess:runtimeAccess}} : {}),
      preferences: {
        ...profile.preferences,
        ...profilePreferences,
      },
