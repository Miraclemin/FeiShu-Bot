...
export async function ensureSecretsGetterWrapper(
  ...
  const bridgeEntry = opts.bridgeEntry ?? process.env.LARK_BRIDGE_CLI_ENTRY ?? process.argv[1] ?? '';
  ...
  const content =
    ...
    `LARK_CHANNEL_HOME=${sq(rootDir)} ${electron ? 'ELECTRON_RUN_AS_NODE=1 ' : ''}exec ${sq(node)} ${sq(bridgeEntry)} secrets get "$@"\n`;
