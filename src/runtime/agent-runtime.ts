import { ExternalAdapter } from '../agent/external-adapter';
...
export function createRuntimeAgent(
  ...
  if (profileConfig.agentKind === 'hermes' || profileConfig.agentKind === 'openclaw') {
    return new ExternalAdapter(profileConfig.agentKind, larkChannel);
  }
  return new ClaudeAdapter({ binary: process.env.LARK_CHANNEL_CLAUDE_BIN, larkChannel });
}
