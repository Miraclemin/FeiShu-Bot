...
export async function startRunFlow(input: StartRunFlowInput): Promise<StartRunFlowResult> {
  ...
  const requestedCwd = group?.workspace ||
  ...
  const policy = evaluateRunPolicy({
    ...
    prompt: workbenchPrompt(group, input.prompt),
    requestedCwd,
  ...
  if (!input.profileConfig.workbench && !resumeFrom && input.capability.agentId === 'claude') {
  ...
  try {
    execution = await input.executor.submit({
      scopeId: input.scopeId,
      policy,
      skills: input.profileConfig.workbench ? { ids: group?.skills ?? [] } : undefined,
      sessionId,
      threadId,
